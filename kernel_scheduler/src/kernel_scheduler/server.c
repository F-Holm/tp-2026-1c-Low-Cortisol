...
void cerrar_hilo_escucha(int sockets_io, t_list* lista_sockets_cpu,
                         pthread_mutex_t* mutex_lista_sockets,
                         pthread_cond_t* cond_fin_hilo_escucha,
                         t_datos_hilo_escucha* datos)
{
  cerrar_io(sockets_io);
  cerrar_cpu(lista_sockets_cpu, mutex_lista_sockets, cond_fin_hilo_escucha);
  free(datos);
}
...
void* hilo_escucha(void* datos_hilo_escucha_void)
{
  ...
  while (true)
  {
    ...
    int socket_fd = esperar_cliente(datos->socket_espera_cpu);
    if (socket_fd <= 0)
      break;
