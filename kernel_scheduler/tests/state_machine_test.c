...
Test(ks_state_machine, a_transition_from_the_wrong_state_is_rejected)
{
  ...
  transition_exec_block(pcb, q); /* expects PS_EXEC -> refused */
...
Test(ks_state_machine, ready_to_exec_only_flips_the_state)
{
  ...
  cr_assert_eq(pcb->state, PS_EXEC);
  destroy_pcb(pcb);
}
...
Test(ks_state_machine, unlock_from_susp_block_goes_to_susp_ready)
{
  ...
  transition_unlock(pcb, q); /* not PS_BLOCK -> susp_block -> susp_ready */
...
Test(ks_state_machine, unlock_from_block_goes_to_ready)
{
  ...
  transition_unlock(pcb, q); /* PS_BLOCK -> ready */
...
Test(ks_state_machine, clear_queues_drains_every_queue)
{
  ...
  t_pcb* r = create_pcb(PS_READY, 0);
  t_pcb* b = create_pcb(PS_BLOCK, 0);
  t_pcb* s = create_pcb(PS_SUSP_READY, 0);
