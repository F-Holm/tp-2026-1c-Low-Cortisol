...
Test(ks_mutex, create_and_add_mutex_reports_duplicates)
{
  ...
  cr_assert_eq(create_and_add_mutex(list, "m1", false, NULL), MR_MUTEX_CREATED);
  cr_assert_eq(create_and_add_mutex(list, "m1", false, NULL),
               MR_MUTEX_NAME_ALREADY_EXISTS);
  cr_assert_eq(create_and_add_mutex(list, "m2", false, NULL), MR_MUTEX_CREATED);
...
Test(ks_mutex, locking_or_unlocking_an_unknown_mutex_is_reported)
{
  ...
  cr_assert_eq(list_mutex_lock(list, "ghost", pcb), MR_MUTEX_NAME_NOT_FOUND);
  cr_assert_eq(list_mutex_unlock(list, "ghost", pcb), MR_MUTEX_NAME_NOT_FOUND);
...
Test(ks_mutex, a_free_mutex_is_granted_to_the_first_caller)
{
  ...
  cr_assert_eq(list_mutex_lock(list, "m", pcb), MR_MUTEX_LOCKED);
  cr_assert_eq(list_mutex_unlock(list, "m", pcb), MR_MUTEX_UNLOCKED);
...
Test(ks_mutex, unlocking_a_mutex_you_do_not_hold_is_rejected)
{
  ...
  t_queues* queues = ks_stub_queues(logger);
  t_mutex_list* list = init_list_mutex();
  t_pcb* owner = create_pcb(PS_EXEC, 0);
  t_pcb* other = create_pcb(PS_EXEC, 0);
...
Test(ks_mutex, priority_inheritance_propagates_transitively_through_a_chain)
{
  ...
  t_pcb* low = create_pcb(PS_EXEC, 5);
  t_pcb* mid = create_pcb(PS_EXEC, 3);
  t_pcb* high = create_pcb(PS_EXEC, 1);

  create_and_add_mutex(list, "mutex_1", true, queues);
  create_and_add_mutex(list, "mutex_2", true, queues);

  cr_assert_eq(list_mutex_lock(list, "mutex_1", low), MR_MUTEX_LOCKED);
  cr_assert_eq(list_mutex_lock(list, "mutex_2", mid), MR_MUTEX_LOCKED);

  // mid blocks on mutex_1 (held by low): direct inheritance, not transitive.
  cr_assert_eq(list_mutex_lock(list, "mutex_1", mid), MR_WAITING_MUTEX);
  ...
  cr_assert_eq(list_mutex_lock(list, "mutex_2", high), MR_WAITING_MUTEX);
...
Test(ks_mutex, a_non_priority_mutex_hands_off_to_waiters_in_fifo_order)
{
  ...
  t_mutex_list* list = init_list_mutex();
  t_pcb* owner = create_pcb(PS_EXEC, 0);
  t_pcb* first = create_pcb(PS_EXEC, 0);
  t_pcb* second = create_pcb(PS_EXEC, 0);

  create_and_add_mutex(list, "m", false, queues);
  list_mutex_lock(list, "m", owner);
  cr_assert_eq(list_mutex_lock(list, "m", first), MR_WAITING_MUTEX);
  cr_assert_eq(list_mutex_lock(list, "m", second), MR_WAITING_MUTEX);

  list_mutex_unlock(list, "m", owner); /* first now holds it, back in READY */
  cr_assert_eq(first->state, PS_READY);
  cr_assert_eq(list_mutex_unlock(list, "m", first), MR_MUTEX_UNLOCKED);
  cr_assert_eq(second->state, PS_READY);
     ...
     priority_mutex_hands_off_to_the_highest_remaining_priority_waiter)
{
  ...
  t_queues* queues = ks_stub_queues_full(logger);
  t_mutex_list* list = init_list_mutex();
  t_pcb* owner = create_pcb(PS_EXEC, 10);
  t_pcb* high = create_pcb(PS_EXEC, 1);
  t_pcb* mid = create_pcb(PS_EXEC, 5);

  create_and_add_mutex(list, "m", true, queues);
  list_mutex_lock(list, "m", owner);
  cr_assert_eq(list_mutex_lock(list, "m", high), MR_WAITING_MUTEX);
  /* mid is not the highest-priority waiter, so its insert must not disturb
   * the owner's already-inherited priority. */
  cr_assert_eq(list_mutex_lock(list, "m", mid), MR_WAITING_MUTEX);
  ...
  cr_assert_eq(list_mutex_unlock(list, "m", owner), MR_MUTEX_UNLOCKED);
  cr_assert_eq(high->state, PS_READY);
  cr_assert_eq(mid->state, PS_BLOCK, "the second waiter keeps waiting");

  cr_assert_eq(list_mutex_unlock(list, "m", high), MR_MUTEX_UNLOCKED);
  cr_assert_eq(mid->state, PS_READY, "the last waiter finally gets it");
...
Test(ks_mutex, destroy_list_mutex_force_releases_remaining_waiters)
{
  ...
  t_mutex_list* list = init_list_mutex();
  t_pcb* owner = create_pcb(PS_EXEC, 0);
  t_pcb* waiter = create_pcb(PS_EXEC, 0);

  create_and_add_mutex(list, "m", false, queues);
  list_mutex_lock(list, "m", owner);
  cr_assert_eq(list_mutex_lock(list, "m", waiter), MR_WAITING_MUTEX);

  destroy_list_mutex(list); /* force-releases `waiter` via transition_unlock */

  cr_assert_eq(waiter->state, PS_READY);
