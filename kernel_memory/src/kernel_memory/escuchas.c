  ...
  while (conexion_estable)
  {
    switch (recibir_operacion(datos_scheduler->socket_scheduler))
    {
      ...
      case OP_PETICION_IO_STDIN:
      {
        ...
        t_peticion_stdin* peticion_stdin = (t_peticion_stdin*)list_get(paquete_stdin, 0);
        char* string_escribir = (char*)list_get(paquete_stdin,1);
        int dir_fisica=traducir_direccion_logica(
        peticion_stdin->pid, peticion_stdin->direccion_logica, peticion_stdin->tamanio_a_leer,
        ...
        int indice = encontrar_stick(dir_fisica, datos_scheduler->sticks_conectados, datos_scheduler->mutex_lista_sockets,
                                 &offset_en_stick);
        pthread_mutex_lock(datos_scheduler->mutex_lista_sockets);
        t_datos_stick* stick_a_escribir_inicial = list_get(datos_scheduler->sticks_conectados, indice);
        int tamanio =  stick_a_escribir_inicial->tamanio_stick - dir_fisica - peticion_stdin->tamanio_a_leer; 
        t_paquete* paquete= crear_paquete(OP_RESPUESTA_STDIN);//NO SE SI ESTA BIEN ESTE CODIGO DE OP
        if(tamanio<0){
          ...
          for (int i = indice+1 ; tamanio_sumado < peticion_stdin->tamanio_a_leer ; i++)
          {
              t_datos_stick* stick_a_escribir = list_get(datos_scheduler->sticks_conectados, i);
              tamanio_sumado =+ stick_a_escribir->tamanio_stick; 
              agregar_a_paquete(paquete,0, sizeof(int));
              char* cadena_cortada = cortar_cadena(stick_a_escribir->tamanio_stick,string_escribir+tamanio_sumado);
              int tamanio_cortado = strlen(cadena_cortada);
              agregar_string_a_paquete(paquete,cadena_cortada);
              agregar_a_paquete(paquete,&tamanio_cortado,sizeof(int));
              enviar_paquete(paquete, stick_a_escribir->socket_stick);
              free(cadena_cortada);
          }
          free(cadena_cortada);
        }else{
