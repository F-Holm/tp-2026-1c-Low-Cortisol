...
int main(int argc, char* argv[])
{
  ...
  t_log* logger = log_create("swap.log", "SWAP", true, log_level); //inicio el log para poder enviar los logs

  ip = config_get_string_value(t_config,"IP"); // Obtengo la IP del archivo de configs

  puerto = config_get_string_value(t_congig, "PUERTO"); //Obtengo el puerto 
  
  socket_swap = crear_conexion(ip,puerto); //Establezco conexión

  log_info("swap.log", "## Conectado a Kernel Memory"); //Loggeo el comentario de conexion iniciada
