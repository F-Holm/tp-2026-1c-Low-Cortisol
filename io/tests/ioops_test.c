...
{
  t_socket* scheduler;
  t_socket* io_socket = io_connected_pair(&scheduler);

  t_io io = {0};
  io.logger = io_quiet_logger();
  io.socket_io = io_socket;

  t_sleep_request request = {.pid = 7, .blocked_time_ms = 5};
  send_buffer(OP_IO_SLEEP_REQUEST, &request, sizeof(request), scheduler);

  cr_assert_eq(receive_op_code(io.socket_io), OP_IO_SLEEP_REQUEST);
  cr_assert(run_sleep(&io));

  cr_assert_eq(receive_op_code(scheduler), OP_SLEEP_RESPONSE);
  char* answer = receive_string(scheduler);
  cr_assert_str_eq(answer, "OK");

  free(answer);
  socket_destroy(io_socket);
  socket_destroy(scheduler);
...
Test(io_run_sleep, reports_failure_when_the_answer_cannot_be_sent)
{
  ...
  socket_shutdown(
      io.socket_io,
      SOCKET_SHUTDOWN_WRITE); /* the reply now fails deterministically */
     ...
     .init = cr_redirect_stdout)
{
  ...
  send_buffer(OP_IO_STDIN_REQUEST, &request, sizeof(request), scheduler);
  ...
  cr_assert_eq(receive_op_code(scheduler), OP_STDIN_RESPONSE);
  char* answer = receive_string(scheduler);
